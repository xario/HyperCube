...
void stopTimers(bool force = false) {
  ...
  for (int i = 0; i < MAX_CLOCKIFY_TIMERS; i++) {
    if (clockifyTimers[i].desiredState != TIMER_RUNNING && clockifyTimers[i].desiredState != TIMER_STARTING) {
      continue;
    }
    
    if (!force && strcmp(clockifyTimers[i].sideId, currentRfid) != 0) {
      #ifdef DEBUG
      Serial.print("The timer side id '");
      Serial.print(clockifyTimers[i].sideId);
      Serial.print("' does not match the current side id '");
    ...
    if (clockifyTimers[i].state == TIMER_PENDING || clockifyTimers[i].state == TIMER_STARTING) {
...
void startTimers() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for(; j < MAX_CLOCKIFY_TIMERS; j++) {
      ...
      Serial.print(", state: ");
      Serial.print(stateToString(clockifyTimers[j].state));
      Serial.println(")");
      #endif
      if (clockifyTimers[j].state != TIMER_STOPPED) {
        if (!preInitTimers) {
          continue;  
        }

        if (clockifyTimers[j].state == TIMER_STOPPING) {
          continue;
        }

        if (clockifyTimers[j].state == TIMER_STOP_SENT) {
          continue;
        }
    ...
    strcpy(clockifyTimers[j].apiKey, jsonExtract(config, "apiKey").c_str());
...
void setup() {
  ...
  //initOTA();
  initNtp();
  if (errCOULD_NOT_ALLOCATE_REQUIRED_MEMORY == xTaskCreatePinnedToCore(reconciliateTimers, "reconciliate", 50000, NULL, 0, NULL, 1)) {
    Serial.println("Could not create reconciliate thread, sleeping forever...");
    while(true) {
      delay(10000);
    }
  }
